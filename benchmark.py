...
if args.version[0] == "0":
    if args.version[-1] == "1":
        data_input = np.load(f"{data_dir}/test_data_{windowsize}_backwards/test_input.npy")
        data_id = np.load(f"{data_dir}/test_data_{windowsize}_backwards/test_id.npy")
        data_output = np.load(f"{data_dir}/test_data_{windowsize}_backwards/test_target.npy")
    else:
        data_input = np.load(f"{data_dir}/test_data_{windowsize}/test_input.npy")
        data_id = np.load(f"{data_dir}/test_data_{windowsize}/test_id.npy")
        data_output = np.load(f"{data_dir}/test_data_{windowsize}/test_target.npy")
    
    if args.amount == -1:
        data_input = np.array(np.split(data_input[:len(data_input)//args.batch_size*args.batch_size],len(data_input)//args.batch_size)[:-1])
        data_id = np.array(np.split(data_id[:len(data_id)//args.batch_size*args.batch_size],len(data_id)//args.batch_size)[:-1])
        ...
        data_input = np.array(np.split(data_input[:len(data_input)//args.batch_size*args.batch_size],len(data_input)//args.batch_size)[:-1])[:args.amount]
        data_id = np.array(np.split(data_id[:len(data_id)//args.batch_size*args.batch_size],len(data_id)//args.batch_size)[:-1])[:args.amount]
    ...
    args.amount = data_input.shape[0]
    print(f"Amount of data: {args.amount}") 
    data = [data_input, data_id]
else:
...
print("Data Loaded")
if args.fp16:
    data = [d.astype("float16") for d in data]
...
input_name = [f.name for f in onnx_model.graph.input]
...
for i,f in enumerate(input_name):
    ...
    input_dict[f] = shape
    if args.version[0] == "0":
        data[i] = data[i].reshape((args.amount, *shape))
...
if args.version[0] == "0":
    if len(input_name) == 1:
        ...
        data = tuple(data[:len(input_name)])
    
output_shapes[0] = args.batch_size


tvm_model, params = tvm.relay.frontend.from_onnx(onnx_model, input_dict, dtype=data_type, freeze_params=True)
print("Model Loaded")
...
if data_type == "float16":
    if args.cluster == "":
        output_file = f"{args.network}-{args.batch_size}-{args.version}-{args.target}-fp16"
    else:
        output_file = f"{args.network}-{args.batch_size}-{args.version}-{args.target}-{args.cluster}-fp16"
else:
    if args.cluster == "":
        output_file = f"{args.network}-{args.batch_size}-{args.version}-{args.target}"
    else:
        output_file = f"{args.network}-{args.batch_size}-{args.version}-{args.target}-{args.cluster}"
    ...
    if type(data_tvm) == tuple:
        ...
        if len(input_name) == 1:
            ...
            for i in range(args.amount):
                for j,f in enumerate(input_name):
                    module.set_input(f, data_tvm[j][i]) # Set the input data
                _benchmark.append(module.benchmark(dev, number=number, repeat=repeat, end_to_end=data_transfer, cooldown_interval_ms=cooldown_interval_ms, min_repeat_ms=args.min_repeat_ms).results)
            _benchmark = np.array(_benchmark).flatten()
        ...
        if list(data_tvm.shape) == input_shapes:
            ...
            print("Evaluate inference time cost done")
            _benchmark = np.array(_benchmark)
            np.savetxt(filename, _benchmark, delimiter=",")
            return _benchmark
        else:
            _benchmark = []
            for i in range(data_tvm.shape[0]):
    ...
    if type(data_tvm) == tuple:
        if len(input_name) == 1:
            ...
            for i in range(len(input_name)):
                module.set_input(input_name[i], data_tvm[i][0])
    else:
        if list(data_tvm.shape) == input_shapes:
            module.set_input(input_name, data_tvm)
        else:
        ...
        for i in range(1, args.batch_size):
            if len(input_name) == 1:
                ...
                for j in range(len(input_name)):
                    module.set_input(input_name[j], data_tvm[j][i])
            module.run()
