...
def evaluate_model(lib, input_shapes, input_name="input_1", data_tvm=None, number=10, repeat=600, data_transfer = False, cooldown_interval_ms=10,filename="benchmark.txt", directory="benchmarks"):
    ...
    if list(data_tvm.shape) != input_shapes:
        ...
        np.savetxt(f"{directory}/"+filename, _benchmark, delimiter=",")
...
def performance(lib, input_shapes, data_tvm=None, true_output = None, input_name="input_1", number=1, repeat=600):
    # Random "data"
    if data_tvm is None:
        data_tvm = tvm.nd.array((np.random.uniform(size=input_shapes)).astype(args.data_type))
    # Random "true_output"
    if true_output is None:
        true_output = tvm.nd.array((np.random.uniform(size=output_shapes)).astype(args.data_type)).asnumpy()
    # Load set_input
    dev = tvm.device(args.target)
    module = runtime.GraphModule(lib["default"](dev))
    if list(data_tvm.shape) != input_shapes:
...
if args.model_tuning and not args.FPGA:
    if log_file is None:
        log_file = "log/"+"%s.log" % (args.network+"_"+args.target+"_"+datetime.datetime.now().strftime('%Y-%m-%d_%H:%M'))
    tuning_option = {
...
if log_file is not None:
    with autotvm.apply_history_best(log_file):
        ...
        if args.code_generation and args.target != "llvm":
            with open("code/"+log_file.replace(".log", "."+file_extension), "w") as f:
                f.write(lib.get_lib().imported_modules[0].get_source())
...
#        if args.plotting:
#            plot_curve(output, 0.95)
        np.save("output/"+output_file, output)